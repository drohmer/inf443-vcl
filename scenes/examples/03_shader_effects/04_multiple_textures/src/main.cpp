...
void load_quads()
{
	...
	GLuint const shader = opengl_create_shader_program( read_text_file("shader/multitexture.vert.glsl"), read_text_file("shader/multitexture.frag.glsl"));

	quad = mesh_drawable_multitexture( mesh_primitive_quadrangle() );

	// Set the two textures and the specific shader
	quad.texture   = opengl_texture_to_gpu(image_load_png("assets/brick.png"));
	quad.texture_2 = opengl_texture_to_gpu(image_load_png("assets/crack.png"));
	quad.shader = shader;
...
void initialize_data()
{
	...
	scene.camera.look_at({0.5f,0.75f,2}, {0.5f,0.5f,0.0f}, {0,1,0});
